...
def get_all_posts(authorized_client , test_posts):
...
def test_create_post_default_published_true(authorized_client,client,test_posts,test_user):                                                           
    res  = authorized_client.post("/posts/", json={"title":"dummy title","content":"dmy content"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "dummy title"
    assert created_post.content == "dmy content"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client , test_posts,test_user):                                              
    res  = client.post("/posts/", json={"title":"dummy title","content":"dmy content"})
    assert res.status_code == 401
